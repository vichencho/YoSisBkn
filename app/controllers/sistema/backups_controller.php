...
class BackupsController extends BackendController {
     ...
     * Método para buscar
     */
    public function buscar($field='denominacion', $value='', $order='order.id.asc', $page=1) {        
        $page               = (Filter::get($page, 'page') > 0) ? Filter::get($page, 'page') : 1;
        $field              = (Input::hasPost('field')) ? Input::post('field') : $field;
        $value              = (Input::hasPost('field')) ? Input::post('value') : $value;
        
        if( strlen($value) <= 2 ) {   
            Flash::warning('Aviso: El patrón de búsqueda no puede tener menos de tres (3) caracteres.');
    ...
    public function restaurar($key='') {                  
        ...
        if(!$id = DwSecurity::getKey($key, 'restaurar_backup', 'int')) {
            return View::ajax();
        }        

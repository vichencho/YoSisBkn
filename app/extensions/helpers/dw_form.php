...
class DwForm extends Form {
     ...
     * @return string
     */
    public static function dbSelect($field, $show=null, $data=null, $blank='Selección', $attrs=null, $value=null, $label='', $help='', $separator='|') {
        ...
        foreach($data as $p) {
            ...
            if(is_array($show)) { //Verifico si se muestran varios campos
                ...
                foreach($show as $item) {
                    if($show[$i] != $slug) {
                        if(isset($p->{$show[$i]})) {
                            $opt.= htmlspecialchars($p->$item, ENT_COMPAT, APP_CHARSET). " $separator ";
                        } else {
                            $opt.= htmlspecialchars($show[$i], ENT_COMPAT, APP_CHARSET). " $separator ";
                        }
                    }
                    $i++;
                }
                $options .= '>' . trim($opt, " $separator "). '</option>';
            } else {
    ...
    public static function hiddenKey($field, $value, $textKey='form_key') {
        $input = self::hidden($field, array('class'=>''), $value).PHP_EOL;
        extract(self::_getFieldName($field));
        ...
        $value = Security::setKey($value, $textKey);
        $input.= self::hidden($field, null, $value).PHP_EOL;
        return $input;
    }

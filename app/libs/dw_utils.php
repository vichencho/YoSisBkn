...
class DwUtils {
     ...
     */
    public static function resaltar($palabra, $texto='==', $color='rojo') {
        $texto = ($texto=='==') ? $palabra : $texto ;
        if ($color == 'negro') {
            $color = 'black';
        ...
        for($i = 0 ; $i < $veces ; $i ++) {
            $palabras_originales[] = '<b style="color: '.$color.'">'.substr($texto,strpos($aux,'%s'),strlen($palabra)).'</b>';

...
Load::models('sistema/recurso_perfil');
class Menu extends ActiveRecord {
    ...
    public function getMenusPorPadre($padre, $order) {
        $columns = 'distinct menu.*, (padre.menu) AS padre, (padre.posicion) AS padre_posicion, recurso.recurso';
        $join = 'LEFT JOIN recurso ON recurso.id = menu.recurso_id ';
    ...
    public static function setMenu($method, $data, $optData=null) {        
        ...
        $recurso_id_aux = empty($obj->recurso_id) ? 0 : $obj->recurso_id;
        $conditions = empty($obj->id) ? "recurso_id=$recurso_id_aux AND visibilidad=$obj->visibilidad" : "recurso_id=$recurso_id_aux AND visibilidad=$obj->visibilidad AND id != '$obj->id'";
        $old = new Menu();

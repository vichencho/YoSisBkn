...
class Recurso extends ActiveRecord {
    ...
    public function getListadoRecursoPorModulo($estado='todos', $order='') { 
        $conditions = '';
        if($estado!='todos') {
            $conditions = ($estado==self::ACTIVO) ? " AND recurso.activo=".self::ACTIVO : " AND recurso.activo=".self::INACTIVO;
        } 

        $sql = '
        SELECT recurso.modulo,menu.posicion
        FROM   recurso, menu
        WHERE  recurso.id = menu.recurso_id
          AND  recurso.id > 1 
          AND recurso.id IS NOT NULL AND recurso.id > 1 ';
        $sql.= $conditions;
        $sql.= '
          AND  recurso.id in (select modulos.recurso_id
                          from  (SELECT modulos.modulo, min(id) as recurso_id
                             FROM  recurso AS modulos
                             WHERE modulos.id IS NOT NULL
                               AND exists (select 1
                                           from menu
                                           where menu.recurso_id = modulos.id
                                          )
                             GROUP BY modulos.modulo) as modulos) ';

        $sql.= 'ORDER BY '.$this->get_order($order,'recurso.id');

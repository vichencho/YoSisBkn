...
// Load::models('params/tipo_nuip', 'sistema/perfil', 'sistema/recurso', 'sistema/recurso_perfil', 'sistema/acceso');

Load::models('params/tipo_nuip', 'personas/persona', 'sistema/perfil', 'sistema/acceso');
class Usuario extends ActiveRecord {
    ...
    public static function getInnerEstado() {
        // $inner = "INNER JOIN (SELECT usuario_id, estado_usuario, descripcion, estado_usuario_at ";
        // $inner.= "            FROM (SELECT * FROM estado_usuario ORDER BY estado_usuario.id DESC ) AS estado_usuario ";
        // $inner.= "            GROUP BY estado_usuario.usuario_id,estado_usuario, descripcion,estado_usuario_at ";
        // $inner.= "           ) AS estado_usuario ON estado_usuario.usuario_id = usuario.id "; 
        // return $inner;
        return "INNER JOIN (SELECT estado_usuario.usuario_id,".
                                  "estado_usuario.estado_usuario,".
                                  "estado_usuario.descripcion,".
                                  "estado_usuario.estado_usuario_at ".
                           "FROM   estado_usuario INNER JOIN (SELECT usuario_id, max(estado_usuario_at) as estado_usuario_at ".
                                                             "FROM   estado_usuario ".
                                                             "GROUP BY usuario_id ) as ultimoestado_usuario ".
                                                 "ON ultimoestado_usuario.usuario_id=estado_usuario.usuario_id and ".
                                                 "ultimoestado_usuario.estado_usuario_at=estado_usuario.estado_usuario_at) AS estado_usuario ".
               "ON estado_usuario.usuario_id = usuario.id ";
    }
    ...
    public static function getUsuarioLogueado() {
        $columnas = 'usuario.*';
        $columnas.= ',persona.*';
        $columnas.= ', perfil.perfil, estado_usuario.estado_usuario';        
    ...
    public function getUsuarioPorPerfil($perfil, $order='order.nombre.asc', $page=0) {
        ...
        $columns = "usuario.*, persona.*, perfil.perfil";        
        $join = 'INNER JOIN perfil ON perfil.id = usuario.perfil_id ';    
        $join.= 'INNER JOIN persona ON persona.id = usuario.persona_id ';       
    ...
    public function getAjaxUsuario($field, $value, $order='', $page=0) {
        ...
        $columns = 'usuario.*, persona.*, perfil.perfil, estado_usuario.estado_usuario, estado_usuario.descripcion';
        $join = self::getInnerEstado();
        $join.= 'INNER JOIN perfil ON perfil.id = usuario.perfil_id ';    
        $join.= 'INNER JOIN persona ON persona.id = usuario.persona_id ';       
